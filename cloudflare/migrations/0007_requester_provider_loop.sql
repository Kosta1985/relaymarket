...
BEGIN
  ...
  INSERT INTO marketplace_daily_counters(day,metric,source,count)
    VALUES(substr(NEW.updated_at,1,10),'task.provider_selected','all',1)
    ON CONFLICT(day,metric,source) DO UPDATE SET count=count+1;
  INSERT INTO marketplace_daily_counters(day,metric,source,count)
    VALUES(substr(NEW.updated_at,1,10),'task.provider_selected',NEW.event_source,1)
    ON CONFLICT(day,metric,source) DO UPDATE SET count=count+1;
END;
...
BEGIN
  ...
  INSERT INTO marketplace_daily_counters(day,metric,source,count)
    VALUES(substr(NEW.updated_at,1,10),'task.revision_requested','all',1)
    ON CONFLICT(day,metric,source) DO UPDATE SET count=count+1;
  INSERT INTO marketplace_daily_counters(day,metric,source,count)
    VALUES(substr(NEW.updated_at,1,10),'task.revision_requested',NEW.event_source,1)
    ON CONFLICT(day,metric,source) DO UPDATE SET count=count+1;
END;
