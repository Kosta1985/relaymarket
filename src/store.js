...
export function launchKpis(){const tasks=state.tasks,agents=state.agents,counters=state.counters,selected=tasks.filter(t=>t.selectedProviderAgentId).length,selectedAccepted=tasks.filter(t=>t.selectedProviderAgentId&&t.acceptedAt).length,accepted=tasks.filter(t=>t.acceptedAt).length,delivered=tasks.filter(t=>t.deliveredAt).length,completed=tasks.filter(t=>t.completedAt).length,disputes=Number(counters.totals['task.disputed']||0);const repeatCount=field=>Object.values(tasks.filter(t=>t.completedAt&&t[field]).reduce((acc,t)=>(acc[t[field]]=(acc[t[field]]||0)+1,acc),{})).filter(count=>count>1).length;const acquisitionSources=Object.entries(counters.bySource||{}).map(([source,values])=>({source,agentRegistrations:Number(values?.['agent.registered']||0),taskCreations:Number(values?.['task.created']||0),matchRequests:Number(values?.['task.match_requested']||0),providerSelections:Number(values?.['task.provider_selected']||0)})).filter(row=>row.agentRegistrations||row.taskCreations||row.matchRequests||row.providerSelections);return{contractVersion:'launch-v1',endpointVerifiedAgents:agents.filter(a=>a.verified).length,independentVerifiedOperators:new Set(agents.filter(a=>a.verified&&a.operatorId).map(a=>a.operatorId)).size,openTasks:tasks.filter(t=>t.status==='open').length,providerSelections:selected,acceptedTasks:accepted,deliveredTasks:delivered,completedTasks:completed,repeatRequesters:repeatCount('requesterAgentId'),repeatProviders:repeatCount('providerAgentId'),matchRequests:Number(counters.totals['task.match_requested']||0),disputeEvents:disputes,conversion:{selectionToAccept:kpiRatioLocal(selectedAccepted,selected),acceptToDeliver:kpiRatioLocal(delivered,accepted),deliverToComplete:kpiRatioLocal(completed,delivered),deliveredToDispute:kpiRatioLocal(disputes,delivered)},medianMinutes:{createToSelection:kpiMedianLocal(tasks,'createdAt','selectedAt'),selectionToAccept:kpiMedianLocal(tasks,'selectedAt','acceptedAt'),acceptToDeliver:kpiMedianLocal(tasks,'acceptedAt','deliveredAt'),createToCompletion:kpiMedianLocal(tasks,'createdAt','completedAt')},acquisitionSources,definitions:{independentVerifiedOperators:'Distinct linked operators represented by endpoint-verified agents. Agents without an operator link are not counted as independently verified operators.',matchRequests:'Requests to the ranking surface, not unique users and not guaranteed qualified matches.',disputeEvents:'Successful task.disputed lifecycle events. A later resolution does not erase the historical dispute event.',conversion:'Observed persisted task timestamps only; null when the denominator is zero.'}};}
...
function authorizeRequester(t,id){if(!t.requesterAgentId)throw problem('task_requester_unbound',409);if(id!==t.requesterAgentId)throw problem('requester_mismatch',403)}
