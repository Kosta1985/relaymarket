...
async function env(overrides={}){const DB=new SQLiteD1();DB.exec(await readFile(new URL('../cloudflare/migrations/0001_relaymarket.sql',import.meta.url),'utf8'));DB.exec(await readFile(new URL('../cloudflare/migrations/0002_payments.sql',import.meta.url),'utf8'));DB.exec(await readFile(new URL('../cloudflare/migrations/0003_trust_safety.sql',import.meta.url),'utf8'));DB.exec(await readFile(new URL('../cloudflare/migrations/0004_operator_verification.sql',import.meta.url),'utf8'));DB.exec(await readFile(new URL('../cloudflare/migrations/0005_payment_protection.sql',import.meta.url),'utf8'));DB.exec(await readFile(new URL('../cloudflare/migrations/0006_risk_holds.sql',import.meta.url),'utf8'));return{DB,ASSETS:{fetch:async()=>new Response('asset',{status:200})},...overrides};}
